...
use chrono::Local;
use serde::Serialize;
use crate::manchester::{manchester_decode, differential_manchester_decode};

#[derive(Debug, Clone, PartialEq)]
pub enum TpmsProtocol {
    Schrader, SchraderEg53ma4, SchraderSmd3ma4,
    Ford, Citroen, Toyota, ToyotaPmv107j, Renault,
    Steelmate, Jansite, JansiteSolar, Elantra, Abarth,
    HyundaiVdo, BmwGen45, BmwGen23, Porsche, Ave,
    TyreGuard, EezTire, GmAftermarket, TrwOok, TrwFsk,
    Airpuxem, SolarTruck,
    All,
}
...
pub fn decode(bits: &[u8], protocol: &TpmsProtocol, min_confidence: u8) -> Option<TpmsPacket> {
    let all: &[fn(&[u8]) -> Option<TpmsPacket>] = &[
        decode_ford, decode_citroen, decode_toyota, decode_toyota_pmv107j,
        decode_renault, decode_schrader_eg53ma4, decode_schrader_smd3ma4,
        decode_schrader_generic, decode_steelmate, decode_jansite,
        decode_jansite_solar, decode_elantra, decode_abarth,
        decode_hyundai_vdo, decode_bmw_gen45, decode_bmw_gen23,
        decode_porsche, decode_ave, decode_tyreguard, decode_eeztire,
        decode_gm_aftermarket, decode_trw_ook, decode_trw_fsk,
        decode_airpuxem, decode_solar_truck,
    ];
...
fn psi_to_kpa(p: f32) -> f32 { p * 6.894_757 }
fn kpa_to_psi(k: f32) -> f32 { k * 0.145_038 }
...
fn score(crc_ok: bool, sane: bool, kpa: f32, temp: Option<f32>) -> u8 {
    ...
    if crc_ok { s += 60; }
    if sane   { s += 20; }
    if (150.0..=350.0).contains(&kpa) { s += 10; }
    if let Some(t) = temp { if (-20.0..=80.0).contains(&t) { s += 5; } }
    s.saturating_add(5).min(100)
}

fn pkt(
    proto: &str, rid: u16, id: u32,
    kpa: f32, temp: Option<f32>,
    batt: Option<bool>, flags: Option<u8>,
    moving: Option<bool>, alarm: Option<bool>,
    raw: &[u8], conf: u8,
) -> TpmsPacket {
    TpmsPacket {
        timestamp: now(), protocol: proto.into(), rtl433_id: rid,
        sensor_id: format!("0x{id:08X}"),
        pressure_kpa: kpa, pressure_psi: kpa_to_psi(kpa),
        temp_c: temp, battery_ok: batt, flags, moving, alarm,
        raw_hex: hex_bytes(raw), confidence: conf,
    }
}
...
fn decode_ford(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = if b[5] & 0x80 == 0 { Some((b[5] & 0x7f) as f32 - 56.0) } else { None };
    let moving = Some(b[6] & 0x44 == 0x44);
    let sane = (0.0..=500.0).contains(&kpa);
    Some(pkt("Ford-VDO", 89, id, kpa, temp, None, Some(b[6]), moving, None,
        &b[..8], score(crc_ok, sane, kpa, temp)))
}
...
fn decode_citroen(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    Some(pkt("Citroen/Peugeot/Fiat", 82, id, kpa, Some(temp),
        Some(b[8] < 0x10), Some(b[5] >> 4), None, None,
        &b[..10], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_toyota(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    Some(pkt("Toyota-PMV-C210", 88, id, kpa, Some(temp), None, Some(b[4]),
        None, None, &b[..9], score(crc_ok && pinv_ok, sane, kpa, Some(temp))))
}

fn decode_toyota_pmv107j(bits: &[u8]) -> Option<TpmsPacket> {
    let mut p = decode_toyota(bits)?;
    p.protocol = "Toyota-PMV-107J".into(); p.rtl433_id = 110; Some(p)
}
...
fn decode_renault(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("Renault/Dacia", 90, id, kpa, Some(temp), None, Some(b[4]),
        None, alarm, &b[..9], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_schrader_eg53ma4(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("Schrader-EG53MA4", 95, id, kpa, Some(temp),
        Some(b[4] & 0x04 == 0), Some(b[4]), None, Some(b[4] & 0x08 != 0),
        &b[..8], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_schrader_smd3ma4(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[6] as f32;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("Schrader-SMD3MA4/3039", 168, id, kpa, Some(temp),
        Some(b[4] & 0x04 == 0), Some(b[4]), None, None,
        &b[..8], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_schrader_generic(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("Schrader", 60, id, kpa, Some(temp), None, Some(b[6]), None, None,
        &b[..8], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_steelmate(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("Steelmate", 59, id, kpa, Some(temp), None, None, None, None,
        &b[..5], score(false, sane, kpa, Some(temp)).min(55)))
}
...
fn decode_jansite(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("Jansite-TY02S", 123, id, kpa, Some(temp),
        Some(b[4] & 0x08 == 0), Some(b[4]), None, Some(b[4] & 0x02 != 0),
        &b[..8], score(crc_ok, sane, kpa, Some(temp))))
}
fn decode_jansite_solar(bits: &[u8]) -> Option<TpmsPacket> {
    let mut p = decode_jansite(bits)?;
    p.protocol = "Jansite-Solar".into(); p.rtl433_id = 180; Some(p)
}
...
fn decode_elantra(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("Hyundai-Elantra-2012", 140, id, kpa, Some(temp), None, Some(b[6]),
        None, None, &b[..8], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_abarth(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[6] as f32 - 50.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("Abarth-124-Spider", 156, id, kpa, Some(temp), None, Some(b[4]),
        None, None, &b[..9], score(xor == 0, sane, kpa, Some(temp))))
}
...
fn decode_hyundai_vdo(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[7] as f32 - 50.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("Hyundai-VDO", 186, id, kpa, Some(temp),
        Some(b[5] & 0x01 == 0), Some(b[5]), None, Some(b[5] & 0x04 != 0),
        &b[..9], score(xor == 0, sane, kpa, Some(temp))))
}
...
fn decode_bmw_gen45(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[6] as f32 - 50.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("BMW-Gen4/5+Audi", 252, id, kpa, Some(temp),
        Some(b[4] & 0x04 == 0), Some(b[4]), None, Some(b[4] & 0x01 != 0),
        &b[..10], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_bmw_gen23(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[6] as f32 - 50.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("BMW-Gen2/3", 257, id, kpa, Some(temp),
        Some(b[4] & 0x04 == 0), Some(b[4]), None, None,
        &b[..8], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_porsche(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[6] as f32 - 40.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("Porsche-Boxster/Cayman", 203, id, kpa, Some(temp), None, Some(b[4]),
        None, None, &b[..10], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_ave(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[5] as f32 - 40.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("AVE-TPMS", 208, id, kpa, Some(temp), None, None, None, None,
        &b[..6], score(xor == 0, sane, kpa, Some(temp))))
}
...
fn decode_tyreguard(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[6] as f32 - 40.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("TyreGuard-400", 225, id, kpa, Some(temp), None, Some(b[4]),
        None, None, &b[..8], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_eeztire(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[6] as f32 - 50.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("EezTire/Carchet/TST-507", 241, id, kpa, Some(temp),
        Some(b[4] & 0x04 == 0), Some(b[4]), None, Some(b[4] & 0x02 != 0),
        &b[..9], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_gm_aftermarket(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[5] as f32 - 40.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("GM-Aftermarket", 275, id, kpa, Some(temp), None, Some(b[6]),
        None, None, &b[..8], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_trw_ook(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[5] as f32 - 40.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("TRW-OOK", 298, id, kpa, Some(temp),
        Some(b[6] & 0x02 == 0), Some(b[6]), None, Some(b[6] & 0x04 != 0),
        &b[..8], score(crc_ok, sane, kpa, Some(temp))))
}
fn decode_trw_fsk(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[5] as f32 - 40.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("TRW-FSK", 299, id, kpa, Some(temp),
        Some(b[6] & 0x02 == 0), Some(b[6]), None, Some(b[6] & 0x04 != 0),
        &b[..8], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_airpuxem(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    let temp = b[6] as f32 - 50.0;
    let sane = (0.0..=400.0).contains(&kpa) && (-40.0..=125.0).contains(&temp);
    Some(pkt("Airpuxem-TYH11", 295, id, kpa, Some(temp), None, Some(b[4]),
        None, None, &b[..9], score(crc_ok, sane, kpa, Some(temp))))
}
...
fn decode_solar_truck(bits: &[u8]) -> Option<TpmsPacket> {
    ...
    Some(pkt("Solar/Truck-TPMS", 201, id, kpa, Some(temp), None, Some(b[4]),
        None, None, &b[..9], score(crc_ok, sane, kpa, Some(temp))))
}
